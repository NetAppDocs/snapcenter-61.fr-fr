...
summary: 'Vous devez créer une connexion à une machine virtuelle de stockage (SVM) et des informations d’identification avant d’utiliser les applets de commande PowerShell pour sauvegarder, restaurer ou cloner des bases de données SAP HANA.' 
---
= Créer une connexion au système de stockage et des informations d'identification à l'aide des applets de commande PowerShell pour la base de données SAP HANA
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous devez créer une connexion à une machine virtuelle de stockage (SVM) et des informations d’identification avant d’utiliser les applets de commande PowerShell pour sauvegarder, restaurer ou cloner des bases de données SAP HANA.
...
.Étapes
. Lancez une session de connexion PowerShell à l’aide de l’applet de commande Open-SmConnection.
+
[listing]
----
...
----
. Ajoutez l’hôte de communication SAP HANA à SnapCenter Server.
+
[listing]
----
PS C:> Add-SmHost -HostName 10.232.204.61 -OSType Windows -RunAsName FinanceAdmin -PluginCode hana
----
. Installez le package et le plug-in SnapCenter pour la base de données SAP HANA sur l'hôte.
+
Pour Linux :
