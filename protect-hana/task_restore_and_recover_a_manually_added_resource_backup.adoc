...
+
.. Si vous sélectionnez *Ressource complète*, tous les volumes de données configurés de la base de données SAP HANA sont restaurés.
+
...
.. Si vous sélectionnez *Niveau de fichier*, vous pouvez alors sélectionner *Tous* ou sélectionner les volumes ou qtrees spécifiques, puis saisir le chemin d'accès associé à ces volumes ou qtrees, séparés par des virgules
+
*** Vous pouvez sélectionner plusieurs volumes et qtrees.
*** Si le type de ressource est LUN, le LUN entier est restauré.
+
